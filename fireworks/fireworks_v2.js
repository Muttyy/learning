  ...
  function windowResized() {
    ...
    this.preStar();// this. でメンバ変数を指定できる
  }
  ...
  function draw() {
    ...
    setGradient(0, 0, width, height, color(0, 0, 0), color(0, 30, 50), Y_AXIS);
    ...
    for (let fw of fireworks) {
      ...
      if (2 === fw.getType || 30000 < fw.getFrame) {// type2:爆発し終わったオブジェクト
        fireworks = fireworks.filter((n) => n !== fw);// 多分indexが現在のもの以上を取り除く？
        continue;
      }
...
// ----------- ここからクラス -------------
  ...
  class FireWork {
    ...
    constructor(x, y, vx, vy, gv) {
      ...
      // 玉の大きさ
      this.w = random(5, 10);// p5.jsの機能

      // 打ち上がる高さ
      this.maxHeight = random(height / 1, height / 8);
      ...
      // 消えてから爆発までの遅延時間
      this.exDelay = random(10, 20);
      // 爆発の大きさ
      this.large = random(1, 15);
      // 爆発の玉の数
